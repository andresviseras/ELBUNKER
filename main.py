...
"""
    ...
    1. UTILIDAD REAL Y CRÍTICA: Todos los roles deben ser piezas clave para el grupo (supervivencia básica, infraestructura, rescate exterior, medicina, defensa, etc.). Evita roles de nicho que sean inútiles por sí solos. Permite que, de forma natural y aleatoria, algunas capacidades se solapen entre varios jugadores para que el grupo tenga que debatir quién es más prescindible. Asume que el refugio tiene suministros básicos temporales. En cada partida, varía el ecosistema de roles de forma impredecible. Algunas partidas deben enfocarse puramente en el aguante a largo plazo (infraestructura, agricultura, purificación); otras deben mezclar roles internos con opciones externas (búsqueda de rescate, expediciones, investigación). Evita roles inútiles y explica tu utilidad directa sin florituras.
    ...
    2. 'habilidad': PRIMERA PERSONA. CONCISA Y AL GRANO (MÁXIMO 3 ORACIONES CORTAS, no superes las 40 palabras en total). Explica tu utilidad vital y, si corresponde a una neutralización, describe tu herramienta física o mecanismo de seguridad específico. Ni una palabra de más.
