...
"""
    ...
    1. UTILIDAD REAL, CRÍTICA Y EVIDENTE: Todos los roles deben ser piezas clave e indiscutibles para el grupo (supervivencia básica, infraestructura crítica, medicina de urgencia, defensa perimetral, rescate exterior, investigación, etc.). PROHIBIDO inventar roles de nicho o reparadores ultra-específicos que parezcan inútiles por sí solos (ej. nada de especialistas en desatascar tuberías con drones o técnicos de insonorización). La utilidad primaria debe ser obvia y de alto valor para cualquier persona nada más leerla. Permite que, de forma natural y aleatoria, algunas capacidades se solapen entre varios jugadores para que el grupo tenga que debatir quién es más prescindible. Asume que el refugio tiene suministros básicos temporales. En cada partida, varía el ecosistema de roles de forma impredecible: algunas partidas deben enfocarse puramente en el aguante a largo plazo (infraestructura, agricultura, purificación), mientras que otras deben mezclar roles internos con opciones externas (búsqueda de rescate, expediciones, investigación). Explica tu utilidad directa sin florituras.
    ...
    Finalmente, diseña la SOLUCIÓN IDEAL SECRETA. Elige exactamente a la mitad de los jugadores que conforman la combinación viable real. Explica la estrategia de victoria subyacente: ya sea una sinergia perfecta de neutralizaciones para sobrevivir indefinidamente, o una estrategia alternativa (ej. lograr un rescate o cura a corto plazo) que justifique asumir ciertos defectos que explotan a largo plazo.
